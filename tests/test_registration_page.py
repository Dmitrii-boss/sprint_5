...
class TestRegistrationPage:
    ...
    def test_registration_success(self, driver):
        ...
        user_name = RandomData.user_name()  # Генерируем случайное имя пользователя
        email = RandomData.email()  # Генерируем случайный email
        password = RandomData.password()  # Генерируем случайный пароль
        driver.find_element(*RegistrationPageLocators.name_input).send_keys(user_name)
    ...
    def test_double_registration_check_error(self, driver):
        ...
        # Генерируем новые данные для первой регистрации
        user_name = RandomData.user_name()
        ...
        driver.find_element(*RegistrationPageLocators.password_input).send_keys(password)
        driver.find_element(*RegistrationPageLocators.registration_btn).click()
        WebDriverWait(driver, 3).until(EC.visibility_of_element_located(AuthPageLocators.login_account_btn))

        # Повторно переходим на страницу регистрации
        driver.get(URLS.REG_PAGE_URL)
        WebDriverWait(driver, 5).until(EC.visibility_of_element_located(RegistrationPageLocators.registration_btn))

        # Генерируем новые данные для второй регистрации
        new_user_name = RandomData.user_name()
        new_email = RandomData.email()
        new_password = RandomData.password()

        # Регистрация второго пользователя
        driver.find_element(*RegistrationPageLocators.name_input).send_keys(new_user_name)
        driver.find_element(*RegistrationPageLocators.email_input).send_keys(new_email)
